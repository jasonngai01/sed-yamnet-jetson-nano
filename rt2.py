...
#load the yamnet >> tflite file
interpreter0 = tf.lite.Interpreter(r'yamnet/yamnet.tflite')
...
embeddings_output_index0 = output_details0[1]['index']
spectrogram_output_index0 = output_details0[2]['index']
...
with open('sed.npy', 'ab') as f:
    while True:
        try:
            ...
            interpreter0.resize_tensor_input(waveform_input_index, [len(waveform)], strict=True)
            interpreter0.allocate_tensors()
            interpreter0.set_tensor(waveform_input_index, waveform)
            interpreter0.invoke()
            scores, embeddings, spectrogram = (
                interpreter0.get_tensor(scores_output_index0),
                interpreter0.get_tensor(embeddings_output_index0),
                interpreter0.get_tensor(spectrogram_output_index0))
            class_scores = tf.reduce_mean(scores, axis=0)
            prediction = scores.mean(axis=0).argmax()
            inferred_class = class_names[prediction] # class map with csv
            top_score = class_scores[prediction] * 100 #percentage of class
            ...
            if uav_prediction == 1:
                ...
                print('NO UAV detected % {:.3f}% of {:12s} '.format(top_score.numpy(),inferred_class))
